...
- Broadest compatibility set: the core Nyctal codebase is not the place to introdude desktop-environment specific wayland extensions. With few exceptions, the only protocols supported by Nyctal directly should be those designated "core" and "stable".
...
It is likely that libseat support will be added in the future. Code in the `drm` and `evdev` directories are highly for example only and while unlikely to change, should not be used as the basis for future support.
...
- `cmd` - a set of example standalone executables 
    - `cmd/nyctal` - a compositor directly accessing linux direct rendering interface and event devices
